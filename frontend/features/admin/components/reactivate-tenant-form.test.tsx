...
describe("ReactivateTenantForm", () => {
  it("opens, submits, and closes the dialog", async () => {
    ...
    const mutate = vi.fn((_tenantId: string, options?: { onSuccess?: () => void }) => {
      options?.onSuccess?.();
    });
    ...
    await user.click(screen.getByRole("button", { name: /reactivate tenant/i }));
    expect(
      screen.getByRole("heading", { name: /reactivate tenant/i }),
    ).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /^reactivate$/i })).toBeDisabled();

    await user.type(screen.getByPlaceholderText(/enter tenant uuid/i), "tenant-1");
