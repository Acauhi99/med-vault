...
import { getCurrentUser, login, register, selectTenant } from "../services/auth";
import {
  loginInputSchema,
  registerInputSchema,
...
export function AuthWorkspace() {
  ...
  }, [currentUserQuery.data, session.accessToken, session.activeTenant?.tenantId]);
  ...
  async function handleLogin(event: SubmitEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result.tenants.length === 1) {
        await pickTenant(result.tenants[0].tenantId, result.accessToken, result.tenants);
      } else {
  ...
  return (
    <div className="w-full max-w-2xl rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl shadow-black/30">
      ...
      <p className="mt-3 text-sm leading-6 text-slate-300">
        The backend returned {tenants.length} tenant{tenants.length === 1 ? "" : "s"}.
      </p>
      ...
      <form className="mt-6 grid gap-4" onSubmit={onSubmit}>
        <label className="grid gap-2 text-sm text-slate-200" htmlFor="tenant-id">
          Tenant
          <select
            id="tenant-id"
            name="tenant-id"
